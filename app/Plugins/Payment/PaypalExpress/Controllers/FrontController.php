...
class FrontController extends RootFrontController
{
    ...
    public function index() {
        ...
        if(empty(session('dataPayment'))) {
            return redirect()->route('cart')->with(["error" => 'No session']);
        }
        $dataPayment = session('dataPayment');
        $dataMapping = [
            'intent' => 'CAPTURE',
            'application_context' => [
                'return_url' => route('paypalexpress.return', ['orderId' => $dataPayment['reference_id']]),
        ...
        try {
            ...
            if ($orderPayment->statusCode == 201) {
                ...
                return redirect()->route('cart')->with(["error" => 'Error status code']);
            }
    ...
    public function getReturn($orderId)
    {
        ...
        if (!empty(session('paypal_express_payment_id'))) {
            ...
            if (empty(request('PayerID')) || empty(request('token'))) {
                return redirect()->route('cart')->with(['error' => 'Link return invalid']);
            }
    ...
    public function processOrder() {
        ...
        if ($orderID && $dataOrder && $arrCartDetail) {
            ...
            return redirect()->route('cart')->with(['error' => 'Data not correct']);
        }
    }
}
