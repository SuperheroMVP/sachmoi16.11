...
class FrontController extends RootFrontController
{
    ...
    public function processOrder(){
        ...
        if (session('orderID')) {
            return redirect()->route('momobasic.index');
        } else {
            return redirect(route('cart'))->with(['error' => trans($this->plugin->pathPlugin.'::lang.order_not_found')]);
        }
    }
    ...
    public function index() {
        ...
        if(!$orderID) {
            return redirect(route('cart'))->with(['error' => trans(trans($this->plugin->pathPlugin.'::lang.order_not_found'))]);
        }
        ...
        $configData = [
            'returnUrl' => route('momobasic.process'), //return  redirect after momo process
            'notifyUrl' => route('momobasic.ipn'),//use for IPN
        ...
        if($dataResponse['errorCode'] == 0) {
            ...
            if($orderID != $dataResponse['orderId']) {
                $msg = trans($this->plugin->pathPlugin.'::lang.data_response_not_match', ['field' => 'orderId']);
                return redirect(route('cart'))->with(['error' => $msg]);
            }
            ...
            if($signature != $dataResponse['signature']) {
                $msg = trans($this->plugin->pathPlugin.'::lang.data_response_not_match', ['field' => 'Signature']);
